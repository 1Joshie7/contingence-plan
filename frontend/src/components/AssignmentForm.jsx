...
export default function AssignmentForm({ initialData, submitLabel, assignmentId, mode = 'create' }) {
  ...
  const buildTestCasePayload = () => {
    const base = {
      assignment: mode === 'edit' ? assignmentId : undefined,
    ...
    if (testCaseForm.test_type === 'stdout') {
      ...
      let args = [];
      try {
        args = JSON.parse(testCaseForm.arguments);
        if (!Array.isArray(args)) throw new Error();
      } catch {
        args = [testCaseForm.arguments];
      }
      return {
        ...base,
        test_type: 'function',
    ...
    <div className="max-w-4xl mx-auto p-6">
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="border rounded-lg p-6 bg-gray-50">
          <h3 className="text-lg font-semibold mb-4">Grading Configuration</h3>
          <div className="mb-6">
            <h4 className="font-medium mb-2">Weights (total: {totalWeight}%)</h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {Object.entries(weights).map(([key, val]) => (
                <div key={key}>
                  <label className="block text-sm text-gray-700 capitalize">{key}</label>
                  <input
                    type="number"
                    name={key}
                    value={val}
          ...
          <div className="mb-6">
            <h4 className="font-medium mb-2">Function Requirements</h4>
            <div className="space-y-2">
              <label className="flex items-center">
                <input
                  ...
                  className="mr-2"
                />
                <span>Require a specific function</span>
              </label>
              {functionRequirements.required && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pl-6">
                  <div>
                    <label className="block text-sm text-gray-700">Function Name</label>
                    <input
                      type="text"
                      name="name"
                  ...
                  <div>
                    <label className="block text-sm text-gray-700">Parameter Count</label>
                    <input
                      type="number"
                      name="param_count"
          ...
          <div className="space-y-2">
            <label className="flex items-center">
              <input
                ...
                className="mr-2"
              />
              <span>Require return statement</span>
            </label>
            <label className="flex items-center">
              <input
                type="checkbox"
            ...
            <label className="flex items-center">
              <input
                ...
                className="mr-2"
              />
              <span>Use pylint for style checking</span>
            </label>
          </div>
        </div>
        ...
        <div className="border-t pt-6 mt-4">
          ...
          <div className="bg-gray-50 p-4 rounded-lg mb-6">
            <h4 className="font-medium mb-2">{editingTestCase ? 'Edit Test Case' : 'Add New Test Case'}</h4>
            <div className="space-y-4">
              <div>
                <label className="block text-gray-700 text-sm mb-1">Test Type</label>
                <select
                  name="test_type"
              ...
              {testCaseForm.test_type === 'stdout' && (
                <div>
                  <label className="block text-gray-700 text-sm mb-1">Input Data (stdin)</label>
                  <textarea
                    name="input_data"
              ...
              {testCaseForm.test_type === 'function' && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-gray-700 text-sm mb-1">Function Name</label>
                    <input
                      type="text"
                      name="function_name"
                  ...
                  <div>
                    <label className="block text-gray-700 text-sm mb-1">Arguments (JSON array)</label>
                    <input
                      type="text"
                      name="arguments"
              ...
              <div className="flex items-center">
                ...
                />
                <label className="ml-2 text-sm text-gray-700">Hidden (not visible to students)</label>
              </div>
          ...
          {mode === 'edit' && loadingTestCases && <p>Loading test cases...</p>}
          {displayTestCases.length === 0 ? (
            <p className="text-gray-500">No test cases yet. Add one above.</p>
          ) : (
                  ...
                  {displayTestCases.map((tc) => (
                    <tr key={tc.id} className="border-t">
                      <td className="px-4 py-2 text-sm">
                        {tc.test_type === 'function' ? 'Function' : 'Stdout'}
                      </td>
                      <td className="px-4 py-2 text-sm">
                        {tc.test_type === 'function' ? (
                          <code>{tc.function_name}({JSON.stringify(tc.arguments)})</code>
                        ) : (
                          <code>{tc.input_data || '(empty)'}</code>
                        )}
                      </td>
                      <td className="px-4 py-2 text-sm whitespace-pre-wrap">{tc.expected_output}</td>
                      <td className="px-4 py-2 text-sm">{tc.is_hidden ? 'Yes' : 'No'}</td>
                      <td className="px-4 py-2 space-x-2">
                        <button onClick={() => editTestCase(tc)} className="text-indigo-600 hover:text-indigo-900">Edit</button>
                        <button onClick={() => deleteTestCase(tc)} className="text-red-600 hover:text-red-900">Delete</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
